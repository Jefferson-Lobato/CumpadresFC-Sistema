...
as $$ declare r public.cash_sessions; begin
  ...
  -- Comandas que permaneceram abertas atravessam a troca de turno.
  update public.orders
     set shift_id = r.id, updated_at = now()
   where status = 'open';
  return r;
end $$;
...
-- ============================================================
-- V2.3 - COZINHA REVERSÍVEL + HISTÓRICO DE STATUS + EXCLUSÃO
-- Execute este bloco após o SQL principal se já houver banco V2.2.
-- ============================================================

create table if not exists public.kitchen_status_history (
  id uuid primary key default gen_random_uuid(),
  kitchen_order_id uuid not null references public.kitchen_orders(id) on delete cascade,
  from_status public.kitchen_status,
  to_status public.kitchen_status not null,
  changed_by uuid references public.profiles(id) on delete set null,
  changed_at timestamptz not null default now()
);

create index if not exists idx_kitchen_status_history_order
  on public.kitchen_status_history(kitchen_order_id, changed_at desc);

alter table public.kitchen_status_history enable row level security;

drop policy if exists kitchen_history_read on public.kitchen_status_history;
create policy kitchen_history_read on public.kitchen_status_history
for select to authenticated
using (public.has_role('admin') or public.has_role('caixa') or public.has_role('cozinha'));

drop policy if exists kitchen_history_insert on public.kitchen_status_history;
create policy kitchen_history_insert on public.kitchen_status_history
for insert to authenticated
with check (public.has_role('admin') or public.has_role('cozinha'));

create or replace function public.set_kitchen_status(
  p_kitchen_order uuid,
  p_status public.kitchen_status
...
as $$
declare
  r public.kitchen_orders;
  old_status public.kitchen_status;
begin
  if not (public.has_role('admin') or public.has_role('cozinha')) then
    raise exception 'Sem permissão para alterar pedidos da cozinha';
  end if;

  select * into r from public.kitchen_orders
  where id = p_kitchen_order
  for update;

  if not found then
    raise exception 'Pedido da cozinha não encontrado';
  end if;
  ...
  if old_status is distinct from p_status then
    insert into public.kitchen_status_history
      (kitchen_order_id, from_status, to_status, changed_by)
    values
      (p_kitchen_order, old_status, p_status, auth.uid());
  end if;
...
revoke all on function public.set_kitchen_status(uuid,public.kitchen_status) from public;
grant execute on function public.set_kitchen_status(uuid,public.kitchen_status) to authenticated;

create or replace function public.delete_open_order(p_order uuid)
...
begin
  if not (public.has_role('admin') or public.has_role('caixa')) then
    raise exception 'Sem permissão para excluir comandas';
  end if;

  select * into r from public.orders where id=p_order for update;
  if not found then raise exception 'Comanda não encontrada'; end if;
  if r.status <> 'open' then raise exception 'Somente comandas abertas podem ser excluídas'; end if;

  update public.orders set status='cancelled', updated_at=now() where id=p_order returning * into r;
  return r;
end;
$$;

revoke all on function public.delete_open_order(uuid) from public;
grant execute on function public.delete_open_order(uuid) to authenticated;

-- Permite que pedidos entregues continuem visíveis durante o turno.
-- O frontend filtra por shift_id e status, removendo apenas CANCELLED.

alter publication supabase_realtime add table public.kitchen_status_history;
